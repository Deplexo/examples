...
A one-page personal website made with HTML and CSS. Customize the introduction, selected work, and contact links. Served by an unprivileged Nginx container with runtime files in /tmp.
...
docker run --rm --read-only --tmpfs /tmp:rw,noexec,nosuid,size=100m -p 3000:3000 --env-file .env static-site
```
...
To export manually, clone `Deplexo/examples` and run `node scripts/export.mjs static-site /path/to/new-app`. Initialize and push that directory to your own Git repository, then import it in Deplexo with the Dockerfile build method and repository root `./`.
