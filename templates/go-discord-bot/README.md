...
A Discord Gateway bot with /ping and /about slash commands. Commands are registered in one development server for fast iteration, with minimal gateway intents and a graceful connection shutdown.

[Deploy with Deplexo](https://deplexo.com/add) · [Deployment guide](https://docs.deplexo.com/guides/discord-bot/)
...
- `DISCORD_GUILD_ID` (required): ID of the development server where commands are registered.

Before starting, install the application in that server. In the [Discord Developer Portal](https://discord.com/developers/applications), enable Guild Install and generate an installation link with the `bot` and `applications.commands` scopes. Open the link and select a server you manage. This starter needs no administrator permission or privileged gateway intents. Enable Developer Mode in Discord, then copy that server's ID for `DISCORD_GUILD_ID`.

## Container

```sh
docker build -t go-discord-bot .
docker run --rm --read-only --tmpfs /tmp:rw,noexec,nosuid,size=100m --env-file .env go-discord-bot
```
...
To export manually, clone `Deplexo/examples` and run `node scripts/export.mjs go-discord-bot /path/to/new-app`. Initialize and push that directory to your own Git repository, then import it in Deplexo with the Dockerfile build method and repository root `./`.
