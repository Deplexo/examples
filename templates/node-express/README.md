...
A small Node.js API with a health endpoint, bounded JSON request bodies, explicit validation, graceful shutdown, and automated HTTP tests.
...
docker run --rm --read-only --tmpfs /tmp:rw,noexec,nosuid,size=100m -p 3000:3000 --env-file .env node-express
```
...
To export manually, clone `Deplexo/examples` and run `node scripts/export.mjs node-express /path/to/new-app`. Initialize and push that directory to your own Git repository, then import it in Deplexo with the Dockerfile build method and repository root `./`.
